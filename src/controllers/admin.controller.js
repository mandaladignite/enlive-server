    ...
    try {
        ...
        const activeCustomers = await User.countDocuments({
            $or: [
                ...
                { _id: { $in: await Order.distinct('userId') } }
            ],
            isActive: true
        });
        const lastMonthActiveCustomers = await User.countDocuments({
            $or: [
                ...
                { _id: { $in: await Order.distinct('userId', { createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth } }) } }
            ],
            isActive: true
        });
        ...
        // Products sold (total quantity from orders)
        const productsSoldResult = await Order.aggregate([
            { $match: { status: { $in: ['delivered', 'completed'] } } },
            { $unwind: '$items' },
            { $group: { _id: null, totalQuantity: { $sum: '$items.quantity' } } }
        ]);
        const productsSold = productsSoldResult.length > 0 ? productsSoldResult[0].totalQuantity : 0;

        const lastMonthProductsSold = await Order.aggregate([
            { 
                $match: { 
        ...
        const lastMonthProductsSoldCount = lastMonthProductsSold.length > 0 ? lastMonthProductsSold[0].totalQuantity : 0;
        const productsChange = lastMonthProductsSoldCount > 0 
            ? ((productsSold - lastMonthProductsSoldCount) / lastMonthProductsSoldCount * 100).toFixed(1)
            : 0;
